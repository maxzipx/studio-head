...
export function getEventDeck(): EventTemplate[] {
  return [
    ...
    // ─── Phase 4: New Arc A — streaming-pivot ────────────────────────────────
    ...
    },
    {
      ...
      body: 'The board wants a formal position: continue theatrical, pivot to streaming, or close gracefully.',
      cooldownWeeks: 14,
      baseWeight: 0.8,
      minWeek: 16,
      buildDecision: ({ idFactory }) => ({
        ...
        options: [
          ...
          {
            id: idFactory('opt'),
            label: 'Transition To Streaming',
            preview: 'Revenue positive. Distributor relationships take the hit.',
            cashDelta: 400_000,
            scriptQualityDelta: 0,
            hypeDelta: 0,
    ...
    },
    {
      ...
      scope: 'studio',
      requiresFlag: 'franchise_committed',
      title: 'Universe Expansion Offer',
      decisionTitle: 'Universe Expansion Offer',
      body: 'A platform offers co-production funding for a connected series expansion of your franchise IP.',
      cooldownWeeks: 18,
      baseWeight: 0.75,
      minWeek: 12,
      buildDecision: ({ idFactory }) => ({
        ...
        category: 'finance',
        title: 'Universe Expansion Offer',
        body: 'The money is real. So is the coordination overhead and distribution friction.',
        weeksUntilExpiry: 1,
        options: [
          {
            id: idFactory('opt'),
            label: 'Accept Co-Production',
            preview: 'Maximum reach. Real distributor friction.',
            cashDelta: 600_000,
            scriptQualityDelta: 0,
            hypeDelta: 0,
            distributorRepDelta: -2,
            audienceDelta: 3,
            talentRepDelta: 1,
            setFlag: 'franchise_tv_active',
          },
          {
            id: idFactory('opt'),
            label: 'License The IP Only',
            preview: 'Moderate revenue. Creative control stays theatrical.',
            cashDelta: 250_000,
            scriptQualityDelta: 0,
            hypeDelta: 0,
            audienceDelta: 1,
          ...
          {
            id: idFactory('opt'),
            label: 'Decline - Protect Theatrical Exclusivity',
            preview: 'Clean. Distributors notice the loyalty.',
            cashDelta: 0,
            scriptQualityDelta: 0,
            hypeDelta: 0,
            distributorRepDelta: 3,
    ...
    {
      id: 'streaming-rights-inquiry',
      category: 'finance',
      scope: 'project',
      targetPhases: ['postProduction', 'distribution'],
      blocksArc: { id: 'streaming-pivot', status: 'resolved' },
      title: 'Streaming Acquisition Offer',
      decisionTitle: 'Streaming Acquisition Offer',
      body: 'A platform makes an unsolicited bid to acquire your upcoming theatrical release for streaming-first distribution. The number is real.',
      cooldownWeeks: 8,
      baseWeight: 1.0,
      minWeek: 6,
      buildDecision: ({ idFactory, projectId, projectTitle, currentWeek }) => ({
        ...
        projectId,
        category: 'finance',
        arcId: 'streaming-pivot',
        arcStage: 0,
        title: 'Streaming Acquisition Offer',
        body: projectTitle
          ? `A platform wants to acquire ${projectTitle} for streaming-first distribution before theatrical.`
          : 'A platform wants to acquire your project for streaming-first distribution.',
        weeksUntilExpiry: 2,
        options: [
          {
            id: idFactory('opt'),
            label: 'Sell Streaming Rights',
            preview: 'Immediate windfall. Theatrical partners will notice.',
            cashDelta: 600_000,
            scriptQualityDelta: 0,
            hypeDelta: 0,
            distributorRepDelta: -3,
          ...
          {
            id: idFactory('opt'),
            label: 'Decline — Stay Theatrical',
            preview: 'Preserves distribution relationships. No cash.',
            cashDelta: 0,
            scriptQualityDelta: 0,
            hypeDelta: 0,
            distributorRepDelta: 2,
    ...
    {
      id: 'exclusive-content-term',
      category: 'finance',
      scope: 'studio',
      requiresArc: { id: 'streaming-pivot', minStage: 1, status: 'active' },
      title: 'Exclusive Content Term Sheet',
      decisionTitle: 'Exclusive Content Term Sheet',
      body: 'The platform returns with a multi-project deal. More money — but it re-routes your entire slate away from theatrical windows.',
      cooldownWeeks: 10,
      baseWeight: 1.0,
      minWeek: 8,
      buildDecision: ({ idFactory, currentWeek }) => ({
        ...
        arcStage: 1,
        title: 'Exclusive Content Term Sheet',
        body: 'The streaming partner is escalating. Accept a multi-project deal or hold the line at a single-title arrangement.',
        weeksUntilExpiry: 2,
        options: [
          {
            id: idFactory('opt'),
            label: 'Accept Multi-Project Deal',
            preview: 'Large upfront. Theatrical relationships may not survive it.',
            cashDelta: 900_000,
            scriptQualityDelta: 0,
            hypeDelta: 0,
            distributorRepDelta: -4,
            audienceDelta: 2,
            setArcStage: 2,
            setFlag: 'streaming_partner',
          },
          {
            id: idFactory('opt'),
            label: 'Counter With Single-Title Only',
            preview: 'Limits exposure. Resolves arc on acceptable terms.',
            cashDelta: 300_000,
            scriptQualityDelta: 0,
            hypeDelta: 0,
    ...
    {
      id: 'streaming-theatrical-crossroads',
      category: 'finance',
      scope: 'studio',
      requiresArc: { id: 'streaming-pivot', minStage: 2, status: 'active' },
      title: 'Streaming or Theatrical: Final Call',
      decisionTitle: 'Streaming or Theatrical: Final Call',
      body: 'The board wants a single answer. Hybrid is off the table. Where does this studio release its films?',
      cooldownWeeks: 12,
      baseWeight: 0.85,
      minWeek: 10,
      buildDecision: ({ idFactory, currentWeek }) => ({
        ...
        arcStage: 2,
        title: 'Streaming or Theatrical: Final Call',
        body: 'Board demands a definitive distribution identity. Commit to streaming-first or return to theatrical priority.',
        weeksUntilExpiry: 1,
        options: [
          {
            id: idFactory('opt'),
            label: 'Commit To Streaming-First',
            preview: 'Structural identity shift. High distributor cost, strong audience reach.',
            cashDelta: 500_000,
            scriptQualityDelta: 0,
            hypeDelta: 0,
            distributorRepDelta: -4,
            audienceDelta: 3,
            setFlag: 'streaming_committed',
            resolveArc: true,
          },
          {
            id: idFactory('opt'),
            label: 'Return To Theatrical Priority',
            preview: 'Breakage fee to exit. Distributor relationships restored.',
            cashDelta: -200_000,
            scriptQualityDelta: 0,
            hypeDelta: 0,
            distributorRepDelta: 4,
            clearFlag: 'streaming_partner',
            resolveArc: true,
          },
          {
            id: idFactory('opt'),
            label: 'Delay The Decision',
            preview: 'No one trusts an indecisive studio.',
            cashDelta: 0,
            scriptQualityDelta: 0,
            hypeDelta: 0,
            talentRepDelta: -2,
